...
Requires healthy peers; records real lint/synthesis and same-fixture RTL mutants.
Next independently audit evidence and perform clean-export compatibility.
"""
from pathlib import Path
import argparse,hashlib,json,subprocess,sys
R=Path(__file__).resolve().parents[2];p=argparse.ArgumentParser(description=__doc__);p.add_argument('--kd28-root',type=Path,required=True);a=p.parse_args();S=R/'build/verification/tl_control_partition';B=S/'checks';B.mkdir(exist_ok=False)
...
source=src[0].read_text();mutations={
 ...
 'mid_field_cut':('&&complete_boundary&&',"&&1'b1&&"),
...
for name,(old,new) in mutations.items():
    ...
    d=B/name;d.mkdir();f=d/'tl_control_partition.v';f.write_text(source.replace(old,new));c=run(name,[sys.executable,str(R/'verification/tl_control_partition/run_rtl.py'),'--label','fault_'+name,'--replace',str(f)]);result=json.loads((S/('fault_'+name)/'results.json').read_text());rows.append(dict(kind='unit_fault',name=name,passed=c.returncode==1 and len(result['results'])==2 and all(x['compile_exit']==0 and x.get('run_exit')==1 for x in result['results'])));print(rows[-1],flush=True)
...
for name in ('lost_cursor','early_source_release','lost_tag_offset','ignore_capacity'):
    f=B/name/'tl_control_partition.v';sources=[f if x.name==f.name else x for x in sorted((R/'rtl/tl').glob('*.v'))]+[R/'rtl/upli/upli_receive_fifo.v',R/'rtl/upli/upli_receive_storage.v']+ext
    for source_tb in sorted((S/'peers_semantics').glob('*/tb.sv')):
...
result=dict(complete=all(x['passed'] for x in rows),results=rows,sources={str(f):hashlib.sha256(f.read_bytes()).hexdigest() for f in src});(B/'results.json').write_text(json.dumps(result,indent=2)+'\n');print('checks complete',result['complete']);raise SystemExit(0 if result['complete'] else 1)
