...
R=Path(__file__).resolve().parents[2];p=argparse.ArgumentParser(description=__doc__);p.add_argument('--kd28-root',type=Path,required=True);p.add_argument('--label',default='checks');p.add_argument('--peers-label',default='peers_semantics');p.add_argument('--replace',type=Path);p.add_argument('--boundary-fault',choices=('missing_guard','truncated_header'),default='missing_guard');a=p.parse_args();S=R/'build/verification/tl_control_partition'
