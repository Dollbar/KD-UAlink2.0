...
def main():
    p=argparse.ArgumentParser(description=__doc__);p.add_argument('--label',required=True);p.add_argument('--pair',default='mapped_pair');p.add_argument('--physical',default='physical_baseline');p.add_argument('--widths',type=int,nargs='+',choices=(8,16),default=[8,16]);a=p.parse_args()
    ...
    for width in a.widths:
        ...
        command=f'cec -T 120 -v "{folder}/gold.blif" "{folder}/gate.blif"';(folder/'cec_command.txt').write_text(command+'\n')
    ...
    result['complete']=len(result['results'])==len(a.widths) and all(r['equivalent'] for r in result['results']);dump(stage/'results.json',result);return 0 if result['complete'] else 1
