...
- [ ] Prove reset establishes equal cursor states after one edge from arbitrary
...
- [ ] Use actual mapped netlist mutations to test state, output and reset obligations;
  retain concrete counterexamples, all failures/timeouts and actual exit codes.
- [ ] Audit register/clock inventory, every compared output bit, all proof denominators
