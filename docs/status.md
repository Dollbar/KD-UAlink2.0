...
进行中的实验：`161ed37` 调整 FIFO 预取资格选择，尚未采用。36 组 FIFO、6 组完整 buffered 组合、36 组发送单元、32 组实际双端、27 组所有权验证及干净克隆检查通过；WIDTH 8 的 30 项测量显示面积和最差 setup 均退化，WIDTH 16 STA 仍运行。以下完整映射/物理结论对应上一已采用基线，不能转记给该候选。详见[实验审查](tl_fifo_issue_selection_review.md)。
