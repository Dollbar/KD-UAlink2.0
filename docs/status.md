...
WIDTH 8/16已完成实际TSMC28标准单元映射，分别为73,677/75,364个单元、6,254/6,574个FF及各64个固定SRAM宏；标准单元面积47,213.082/48,461.994 µm²，不含未表征SRAM面积。WIDTH 8的五角×三种synthetic SRAM视图×两时钟模式共30组STA测量完成：640 ps全部未收敛，6.4 ns参考模式13/15通过，最差setup/hold为−3.217651/−0.008036 ns。

WIDTH 16的STA仍在原后台任务中运行，60组整体审计尚未完成。12项报告解析测试与6项实际映射库存检查在普通及优化Python模式通过。完整映射等价、主频收敛、真实宏签核和完整双IP仍开放；详见 [当前物理检查点](tl_tx_prepared_timing_review.md)。
