...
- [ ] Qualify complete output/next-state comparison with actual mapped-cell mutation, then run healthy CEC for WIDTH 8/16.
- [ ] Prove reset and independent dormant preparation payload cases, so an assumed equal-state CEC is not mislabeled as reset-reachable sequential equivalence.
- [ ] Audit all scope boundaries and bind evidence. Full payload/protocol reference correspondence and physical timing closure remain separate Goal obligations.
