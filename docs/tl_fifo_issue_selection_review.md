...
The physical matrix uses unchanged actual standard-cell libraries, synthetic SRAM views, periods and IO budgets. WIDTH 8 completes 30 measurements at standard-cell area 45,184.860 µm², worst setup −1.809450 ns and worst hold −0.008036 ns. Against the adopted baseline, area increases 0.880% and worst setup worsens 3.065 ps; the width still closes only the same 13 reference-period profiles. Actual mapped Q aliases bind its worst path from Request Header FIFO `reg_head[123]` to Response Header FIFO `cnt_unread[1]`, through the real header-taken and issue signals. WIDTH 16 mapping reports area 47,284.650 µm², while its STA is ongoing. Full physical auditing and the adoption decision remain pending; the WIDTH 8 evidence does not support this candidate.
