...
- [ ] Run `run_storage_visibility.py --top fifo --depths 1 2 3 5 129 257 --label fifo_issue_fifo` and the actual buffered comparison with `--top buffered --kd28-root PATH --widths 8 16 --depths 1 2 3 --label fifo_issue_buffered`. Expected outputs are immutable snapshots, full-state graphs, reset/CEC logs and complete `results.json` under `build/verification/tl_tx_qualification/`.
- [ ] Run strict FIFO lint, six actual SRAM FIFO depths, production unit and 32 actual peer configurations using fresh `fifo_issue_*` labels. If initial checks pass, measure the actual production top with `run_physical.py --lib-root LIB_ROOT --kd28-root PATH --sta STA_PATH --widths 8 16 --label fifo_issue_physical`; preserve its 60-profile results even if setup/hold remain open.
- [ ] Independently audit the full physical matrix in ordinary and optimized Python. Compare actual setup, hold, area and FF/macro inventory with `header_visibility_clean_physical`. Reject unsupported or regressing candidates; retain candidate snapshots and restore exact baseline RTL if rejected.
- [ ] If adoption is supported, complete all WIDTH 8–16 buffered and ownership checks, actual mapped reset/cursor/dormant/partition/fault correspondence, normal/optimized evidence audits and clean-clone checks before recording adoption. Otherwise document the measured rejection and use its actual critical path to choose the next repair.
