...
- [ ] Establish the ready-selection identity against the actual RTL and retain a failing negative that incorrectly ignores the held class.
- [ ] Make the bounded channel-only change; preserve public ports, all eight state bits and cycle behavior. Complete WIDTH 8–16 public-output/next-state and reset equivalence against the frozen RTL reference.
- [ ] Rerun independent channel vectors, actual prepared-transmit SRAM peers/ownership checks and the relevant real wiring faults. Audit current source identity and healthy/failing outcomes separately.
